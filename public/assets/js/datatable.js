...
(function (factory) {
  ...
  function _fnCreateTr(oSettings, iRow, nTrIn, anTds) {
    ...
    if (row.nTr === null) {
      ...
      for (i = 0, iLen = oSettings.aoColumns.length; i < iLen; i++) {
        ...
        if (
          create ||
          ((oCol.mRender || oCol.mData !== i) && (!$.isPlainObject(oCol.mData) || oCol.mData._ !== i + '.display'))
        ) {
  ...
  _Api.extend = function (scope, obj, ext) {
    ...
    for (i = 0, ien = ext.length; i < ien; i++) {
      ...
      // Value
      obj[struct.name] =
        struct.type === 'function' ? _api_scope(scope, struct.val, struct) : struct.type === 'object' ? {} : struct.val;
  ...
  DataTable.feature.register(
    ...
    function (settings, opts) {
      ...
      var div = $('<div/>').addClass(classes.container).append(str.replace('_MENU_', '<span></span>'));
