                    ...
                    <div class="overflow-x-auto">
                        <table id="example"  class="w-full hover divide-neutral-200 dark:divide-neutral-700">
                            ...
                            <tr class="dark:hover:bg-neutral-800">
                                ...
                                <th scope="col" class="px-6 py-3 text-xs font-medium text-gray-500 uppercase text-start">Is_active</th>
                            ...
                            @foreach($leave_types as $leave_type)
                                <tr class="cursor-pointer text-start dark:hover:bg-neutral-800">
                                    <td class="px-6 py-4 text-sm text-gray-800 whitespace-nowrap dark:text-gray-200">{{ $leave_type->id }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-800 whitespace-nowrap dark:text-gray-200">{{ $leave_type->name }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-800 whitespace-nowrap dark:text-gray-200">{{ $leave_type->is_active }}</td>
                                    {{-- {{ $department->empDepartment->name ?? '' }} --}}
                                    <td class="px-6 py-4 text-sm font-medium whitespace-nowrap text-end">
                                        <div class="flex gap-3">
                                            <div class="inline-block hs-tooltip">
                                                <a href="{{ route('leave-type.edit', $leave_type->id) }}" class="inline-flex items-center px-2 py-2 text-sm font-semibold text-gray-500 border rounded-lg hs-tooltip-toggle border-neutral-300 gap-x-2 hover:border-red-600 hover:text-red-600 disabled:opacity-50 disabled:pointer-events-none dark:border-neutral-700 dark:text-gray-400 dark:hover:text-red-500 dark:hover:border-red-600">
                                            ...
                                            <div class="inline-block hs-tooltip">
                                                <a href="{{route('leave-type.delete', $leave_type->id)}}" class="inline-flex items-center px-2 py-2 text-sm font-semibold text-gray-500 border rounded-lg hs-tooltip-toggle border-neutral-300 gap-x-2 hover:border-red-600 hover:text-red-600 disabled:opacity-50 disabled:pointer-events-none dark:border-neutral-700 dark:text-gray-400 dark:hover:text-red-500 dark:hover:border-red-600">
